#!/usr/bin/env python3
"""Kenya Security & Safety PSA Corpus Scraper.

Collects public security advisories, road safety warnings, disaster alerts, and crime prevention notices
from licensed Kenyan security agencies (NPS, DCI, NTSA, NCIC, KBC, Mkulima Mbunifu).
...
Strictly enforces:
  ...
  - Single CSV output target (data/kenya_security_psa.csv)
  - Automatic temporary buffer fallback if data/kenya_security_psa.csv is locked by an editor
"""
...
IMPERATIVE_STARTERS = {
    # English Security & Safety Action Verbs
    "avoid", "report", "call", "dial", "ensure", "check", "verify", "do not", "don't",
    "never", "stop", "remain", "stay", "obey", "follow", "adhere", "surrender", "inform",
    "notify", "contact", "alert", "warn", "be vigilant", "take care", "protect", "guard",
    "secure", "lock", "inspect", "comply", "submit", "register", "renew", "keep", "maintain",
    "refrain", "watch out", "pay attention", "evacuate", "seek", "apply", "register",
    # Swahili Security & Safety Action Verbs
    "tahadhari", "tangazo", "hakikisha", "epuka", "ripoti", "piga", "toa", "ilani", "ushauri",
    "taarifa", "sajili", "linda", "zingatia", "fuata", "toa taarifa", "jiandae", "baki",
    "kaa", "tuma", "lipa", "angalia", "thibitisha", "hudhuria", "pakua", "ingia", "pata",
    "usifanye", "ondoa", "zuia", "dhibiti", "punguza", "safisha", "tumia", "usitumie",
}

SECURITY_TERMS = {
    # English
    "security", "police", "crime", "safety", "alert", "warning", "advisory", "notice",
    "emergency", "dci", "ntsa", "traffic", "road safety", "disaster", "fraud", "scam",
    "vigilance", "suspicious", "cohesion", "peace", "hotline", "threat", "accident",
    # Swahili
    "usalama", "polisi", "uhalifu", "tahadhari", "tangazo", "dharura", "barabara", "ajali",
    "wizi", "utapeli", "amani", "ripoti", "piga simu", "ugomvi", "janga", "mafuriko",
}

SECURITY_PSA_SIGNAL_PATTERNS = [
    r"^(Alert|Warning|Advisory|Notice|Security Notice|Public Notice|Road Safety Notice|Tahadhari|Tangazo|Taarifa|Ilani|Ushauri):",
    r"\b(report suspicious|call 999|call 112|call 0800|dial|road safety|obey traffic|avoid traveling|be vigilant|report crime|report to police)\b",
    r"\b(tahadhari ya usalama|piga simu|toa taarifa polisi|hakikisha usalama|epuka eneo|zingatia sheria za barabarani)\b",
]
...
def clean_text_prefix(text: str) -> str:
    ...
    cleaned = re.sub(r"^(alert|warning|advisory|notice|security notice|public notice|tahadhari|tangazo|taarifa|ilani|ushauri)\b[\s:]*", "", cleaned).strip()
    return cleaned
...
def record_id(url: str, lang_code: str, psa_text: str) -> str:
    ...
    return f"KSECPSA-{digest}"


def is_security_psa(text: str, lang_code: str = "en") -> bool:
    """Check if statement is a security/safety PSA starting directly with directive action."""
    lowered = text.casefold()
    ...
    return any(term in lowered for term in SECURITY_TERMS) or any(re.search(pat, lowered, re.I) for pat in SECURITY_PSA_SIGNAL_PATTERNS)


def format_security_psa_text(text: str, psa_type: str) -> str:
    """Format extracted text into standard alert/advisory statement."""
    clean_text = re.sub(r"^[\w\s\(\)\/]+:\s*", "", text).strip()
    if re.match(r"^(Alert|Warning|Advisory|Notice|Security Notice|Tahadhari|Tangazo|Taarifa|Ilani|Ushauri):", clean_text, re.I):
        return clean_text
    lowered = clean_text.lower()
    if any(k in lowered for k in ["tahadhari", "danger", "hazard", "threat", "evacuate", "flood"]):
        return f"Warning: {clean_text}"
    elif any(k in lowered for k in ["traffic", "road", "ntsa", "speed", "helmet", "seatbelt"]):
        return f"Road Safety Advisory: {clean_text}"
    elif any(k in lowered for k in ["scam", "fraud", "dci", "cyber", "phishing"]):
        return f"Security Advisory: {clean_text}"
    elif any(k in lowered for k in ["report", "police", "hotline", "999", "112"]):
        return f"Public Notice: {clean_text}"
    return f"Advisory: {clean_text}"


def classify_security_psa(text: str, lang_code: str) -> str:
    lowered = text.casefold()
    if any(term in lowered for term in ("road", "traffic", "ntsa", "speed", "driver", "vehicle", "ajali", "barabara")):
        return "Road Safety Advisory"
    elif any(term in lowered for term in ("scam", "fraud", "dci", "cyber", "phishing", "utapeli", "wizi")):
        return "Crime Prevention Advisory"
    elif any(term in lowered for term in ("disaster", "flood", "fire", "emergency", "janga", "mafuriko")):
        return "Disaster Emergency Alert"
    elif any(term in lowered for term in ("police", "nps", "report", "hotline", "polisi", "ripoti")):
        return "Public Security Notice"
    return "Public Security Advisory"
...
def detect_language(text: str, page_lang: str = "") -> tuple[str, str]:
    ...
    sw_unique = {
        "tahadhari", "tangazo", "hakikisha", "epuka", "ripoti", "piga", "toa", "ilani", "ushauri",
        "taarifa", "sajili", "linda", "zingatia", "fuata", "jiandae", "baki", "kaa", "tuma", "lipa",
        "angalia", "thibitisha", "hudhuria", "pakua", "ingia", "pata", "usifanye", "ondoa", "zuia",
        "dhibiti", "punguza", "safisha", "tumia", "usitumie", "polisi", "uhalifu", "usalama", "barabara",
        "ajali", "wizi", "utapeli", "amani", "ugomvi", "janga", "mafuriko", "katika", "lakini", "watu",
    }
    if any(word in sw_unique for word in words) or (page_lang and page_lang.lower().startswith("sw")):
        return "Swahili", "sw"
    en_markers = {"the", "and", "of", "to", "in", "for", "with", "police", "security", "safety", "crime", "alert", "notice", "warning", "report"}
    if sum(word in en_markers for word in words) >= 2 or re.search(r"\b(the|and|with|from|police|security|safety)\b", text.casefold()):
        return "English", "en"
    return "Unknown", "und"
...
def windows_trust_bundle() -> Path:
    ...
    for line in result.stdout.splitlines():
        try:
            ...
            continue
    bundle = Path(tempfile.gettempdir()) / "kenya-sec-psa-windows-trusted-roots.pem"
            ...
            if page["blocks"]:
                ...
                for block in page["blocks"]:
                    ...
                    if code not in {"en", "sw"}:
                        continue
                    psa_type = classify_security_psa(f"{page['title']} {block}", code)
                    psa = format_security_psa_text(block, psa_type)
                    yield {
                        ...
                        "Review_Status": "candidate — provenance and licence verified; review Security & Safety PSA scope before model training",
                    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sources", default="security_sources.json", type=Path)
    parser.add_argument("--output", default="data/kenya_security_psa.csv", type=Path)
    parser.add_argument("--report", default="data/security_collection_report.json", type=Path)
    ...
    parser.add_argument("--user-agent", default="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) KenyaSecPSACorpusBot/0.1 (research contact: replace-with-your-email@example.org)")
...
def main() -> int:
    ...
    logging.info("=" * 60)
    logging.info("RUN SUMMARY FOR SECURITY DOMAIN:")
