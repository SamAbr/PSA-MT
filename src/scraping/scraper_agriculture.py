...
logger = logging.getLogger("scraper_agriculture")
...
def fetch_html(url: str, use_selenium: bool = False, timeout: int = 15) -> str:
    """Fetch the raw HTML content of a URL (requests, local file://, or Selenium fallback)."""
...
def parse_html_elements(html_content: str) -> list:
    """Parse HTML and extract title, headings, paragraphs, and list items in document order."""
    elements = []
    try:
...
def main():
    parser = argparse.ArgumentParser(description="PSA Agriculture Scraper")
    parser.add_argument(
        "--url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom output CSV path (default: data/raw/scraped/agriculture.csv)"
    )
    parser.add_argument(
        "--selenium",
        action="store_true",
        help="Use Selenium webdriver fallback"
    )
    
    args = parser.parse_args()
    
    # Resolve default output path relative to script location
    if args.output is None:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(script_dir, "..", ".."))
        output_path = os.path.join(project_root, "data", "raw", "scraped", "agriculture.csv")
    else:
        output_path = args.output
        
    logger.info("=" * 60)
    logger.info("Starting Agriculture Scraper")
    ...
    try:
        ...
        logger.info("Agriculture Scraper execution completed successfully.")
    except Exception as e:
